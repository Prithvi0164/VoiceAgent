...
def ask_question(assistant_id, thread_id, question, user_name):
    ...
    if last_message:
        ...
        st.write(f"Assistant updated with Vector Store ID: {response_text}")
...
def initialize():
    ...
    st.write(f"Assistant updated with Vector Store ID: {vector_store_id}")

    # Create a thread for interaction
    thread_id = create_thread()
    st.write(f"Thread created with ID: {thread_id}")
...
def main(assistant_id, thread_id):
    ...
    speak(" My name is Cloudy from CloudPoint. What is your name?")
...
def streamlit_interface():
    st.title("Voice Agent Assistant")
    st.markdown("<p style='color:red;'>To Stop Conversation say :   Exit </p>", unsafe_allow_html=True)
    ...
    if not st.session_state["assistant_active"]:
        if st.button("Start Assistant"):
            st.session_state["assistant_id"], st.session_state["thread_id"] = initialize()
            st.session_state["assistant_active"] = True
            st.write("Assistant has started. You can now interact.")
            main(st.session_state["assistant_id"], st.session_state["thread_id"])  # Start the Q&A session

    if st.session_state["assistant_active"]:
        if st.button("Stop Assistant"):
            st.session_state["assistant_active"] = False
            st.write("Assistant has stopped.")
